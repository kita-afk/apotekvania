...
class Example extends CI_Controller
{
	...
	{
		parent::__construct();
		var_dump($this->load->model('apotek_data'));
		die;
	...
	public function login_masuk()
	{
		...
		$result =$this->db->query("SELECT * FROM user WHERE username = '$username' AND pass = '$password'")->row_array();
		if($result)  {
			$sesi = [
				'login' =>true,
				'id_user'	=>	$result['id'],
			...
			document.location.href = '../example/login';
			</script>";
		}
	}
	...
	function cari_pembelian() {
		...
		if($kategori == "semua") {
			$main = $this->db->query("SELECT * FROM table_purchase LEFT JOIN table_med ON table_med.nama_obat = table_purchase.nama_obat WHERE  tgl_beli = '$waktu'")->num_rows();
			$main_satu = $this->db->query("SELECT * FROM table_purchase LEFT JOIN table_med ON table_med.nama_obat = table_purchase.nama_obat WHERE  tgl_beli = '$waktu'")->result_object();
		} else {
			$main = $this->db->query("SELECT * FROM table_purchase LEFT JOIN table_med ON table_med.nama_obat = table_purchase.nama_obat WHERE nama_kategori = '$kategori' AND tgl_beli = '$waktu'")->num_rows();
			$main_satu = $this->db->query("SELECT * FROM table_purchase LEFT JOIN table_med ON table_med.nama_obat = table_purchase.nama_obat WHERE nama_kategori = '$kategori' AND tgl_beli = '$waktu'")->result_object();
		}
		if($main) {
			$data['table_purchase'] = $main_satu;
			$data['cari'] = true;
			$data['start'] =	$waktu;
			...
			echo "<script>
				...
				document.location.href = '../example/table_purchase';
			</script>";
			exit;
		}
	}
	...
	function cari_penjualan() {
		...
		$waktu = $this->input->post("waktu");
		if($kategori == "semua") {
			$main = $this->db->query("SELECT * FROM table_invoice LEFT JOIN table_med ON table_med.nama_obat = table_invoice.nama_obat WHERE  tgl_beli = '$waktu'")->num_rows();
			$main_satu = $this->db->query("SELECT * FROM table_invoice LEFT JOIN table_med ON table_med.nama_obat = table_invoice.nama_obat WHERE  tgl_beli = '$waktu'")->result_object();
		} else {
			$main = $this->db->query("SELECT * FROM table_invoice LEFT JOIN table_med ON table_med.nama_obat = table_invoice.nama_obat WHERE nama_kategori = '$kategori' AND tgl_beli = '$waktu'")->num_rows();
			$main_satu = $this->db->query("SELECT * FROM table_invoice LEFT JOIN table_med ON table_med.nama_obat = table_invoice.nama_obat WHERE nama_kategori = '$kategori' AND tgl_beli = '$waktu'")->result_object();
		}
		if($main) {
			$data['table_invoice'] = $main_satu;
			$data['cari'] = true;
			$data['start'] =	$waktu;
			...
			echo "<script>
				alert('Data Tidak Ditemukan');
				document.location.href = '../example/table_invoice';
			</script>";
			exit;
		}
	}
	...
	function add_invoice(){
		...
		$rt = $this->db->query("SELECT * FROM table_med WHERE nama_obat = '$val' ORDER BY id_obat DESC")->row_array();
		$t = $this->db->query("SELECT * FROM table_med WHERE nama_obat = '$val'")->row_array();
		$yy = $this->db->query("SELECT * FROM stok WHERE nama_barang = '$val' AND status_id = 'penjualan' ORDER BY id DESC")->row_array();
		if($yy) {
			$maint = [
						'nama_barang'	=>	$val,
						'satuan'	=>	$rt['unit'],
						// 'pemasukan'	=>	null,
						'pengeluaran'	=>	$yy['pengeluaran'] + $subtotal[$key],
						'stok_terakhir'	=>	$rt['stok'] - $banyak[$key],
						'tanggal'	=>	$tgl_beli,
						'kategori'	=>	$rt['nama_kategori'],
						'status_id'	=>	'penjualan'
					];
					$this->db->where('nama_barang',$val)->where('status_id','penjualan')->order_by("id","desc")->update("stok",$maint);
		} else {
			$maint = [
				'nama_barang'	=>	$val,
				'satuan'	=>	$rt['unit'],
				'pemasukan'	=>	null,
				'pengeluaran'	=>	$subtotal[$key],
				'stok_terakhir'	=>	$rt['stok'],
				'tanggal'	=>	$tgl_beli,
				'kategori'	=>	$rt['nama_kategori'],
				'status_id'	=>	'penjualan'
			];
			$this->db->insert("stok",$maint);
		}
		}
	...
	function add_purchase(){
		...
		$rt = $this->db->query("SELECT * FROM table_med WHERE nama_obat = '$val' ORDER BY id_obat DESC")->row_array();
		$t = $this->db->query("SELECT * FROM table_med WHERE nama_obat = '$val'")->row_array();
		$yy = $this->db->query("SELECT * FROM stok WHERE nama_barang = '$val' AND status_id = 'pembelian' ORDER BY id DESC")->row_array();
		if($yy) {
			$maint = [
						'nama_barang'	=>	$val,
						'satuan'	=>	$rt['unit'],
						'pemasukan'	=>	$yy['pemasukan'] + $subtotal[$key],
						'stok_terakhir'	=>	$rt['stok'] - $banyak[$key],
						'tanggal'	=>	$tgl_beli,
						'kategori'	=>	$rt['nama_kategori'],
						'status_id'	=>	'pembelian'
					];
					$this->db->where('nama_barang',$val)->where('status_id','pembelian')->order_by("id","desc")->update("stok",$maint);
		} else {
			$maint = [
				'nama_barang'	=>	$val,
				'satuan'	=>	$rt['unit'],
				'pemasukan'	=>		$subtotal[$key],
				// 'pengeluaran'	=>,
				'stok_terakhir'	=>	$rt['stok'],
				'tanggal'	=>	$tgl_beli,
				'kategori'	=>	$rt['nama_kategori'],
